...
Nous (旧 MemoryMCP) は MCP (Model Context Protocol) 準拠の永続メモリサーバー。RAG検索とメタデータフィルタリングで、Personaごとの記憶を管理します。
...
```bash
docker run -d --name nous -p 26262:26262 \
  -v $(pwd)/data:/data \
  ghcr.io/solidlime/nous:latest
```
...
`NOUS_` プレフィックスを使って任意の設定値を環境変数で上書きできます:

```bash
export NOUS_EMBEDDINGS_MODEL=cl-nagoya/ruri-v3-30m
export NOUS_QDRANT_URL=http://localhost:6333
export NOUS_SERVER_PORT=26262
```
...
|---------|------------|----------|------|
| `NOUS_DATA_DIR` | - | `./` (Docker: `/data`) | データディレクトリ |
| `NOUS_CONFIG_PATH` | - | `data/config.json` | 設定ファイルパス |
| `NOUS_LOG_FILE` | - | `data/logs/memory_operations.log` | ログファイルパス |
| `NOUS_EMBEDDINGS_MODEL` | `embeddings_model` | `cl-nagoya/ruri-v3-30m` | 埋め込みモデル |
| `NOUS_EMBEDDINGS_DEVICE` | `embeddings_device` | `cpu` | デバイス (cpu/cuda) |
| `NOUS_RERANKER_MODEL` | `reranker_model` | `hotchpotch/japanese-reranker-xsmall-v2` | Rerankerモデル |
| `NOUS_RERANKER_TOP_N` | `reranker_top_n` | `5` | Reranker候補数 |
| `NOUS_SENTIMENT_MODEL` | `sentiment_model` | `cardiffnlp/twitter-xlm-roberta-base-sentiment` | 感情分析モデル |
| `NOUS_SERVER_HOST` | `server_host` | `0.0.0.0` | サーバーホスト |
| `NOUS_SERVER_PORT` | `server_port` | `26262` | サーバーポート |
| `NOUS_TIMEZONE` | `timezone` | `Asia/Tokyo` | タイムゾーン |
| `NOUS_RECENT_MEMORIES_COUNT` | `recent_memories_count` | `5` | get_context表示件数 |
| `NOUS_QDRANT_URL` | `qdrant_url` | `http://localhost:6333` | Qdrant接続URL |
| `NOUS_QDRANT_API_KEY` | `qdrant_api_key` | `None` | Qdrant APIキー |
| `NOUS_QDRANT_COLLECTION_PREFIX` | `qdrant_collection_prefix` | `memory_` | Qdrantコレクションプレフィックス |
| `NOUS_SUMMARIZATION_ENABLED` | `summarization.enabled` | `True` | 要約機能有効化 |
| `NOUS_SUMMARIZATION_USE_LLM` | `summarization.use_llm` | `False` | LLM要約 (False=統計要約) |
| `NOUS_SUMMARIZATION_FREQUENCY_DAYS` | `summarization.frequency_days` | `1` | 要約頻度（日数） |
| `NOUS_SUMMARIZATION_MIN_IMPORTANCE` | `summarization.min_importance` | `0.3` | 要約対象最小重要度 |
| `NOUS_SUMMARIZATION_IDLE_MINUTES` | `summarization.idle_minutes` | `30` | 自動要約トリガーのアイドル分数 |
| `NOUS_SUMMARIZATION_CHECK_INTERVAL_SECONDS` | `summarization.check_interval_seconds` | `3600` | 自動要約チェック間隔（秒） |
| `NOUS_SUMMARIZATION_LLM_API_URL` | `summarization.llm_api_url` | `None` | LLM API URL |
| `NOUS_SUMMARIZATION_LLM_API_KEY` | `summarization.llm_api_key` | `None` | LLM APIキー |
| `NOUS_SUMMARIZATION_LLM_MODEL` | `summarization.llm_model` | `anthropic/claude-3.5-sonnet` | LLMモデル名 |
| `NOUS_SUMMARIZATION_LLM_MAX_TOKENS` | `summarization.llm_max_tokens` | `500` | 最大トークン数 |
| `NOUS_SUMMARIZATION_LLM_PROMPT` | `summarization.llm_prompt` | `None` | カスタム要約プロンプト |
| `NOUS_VECTOR_REBUILD_MODE` | `vector_rebuild.mode` | `idle` | リビルドモード (idle/manual) |
| `NOUS_VECTOR_REBUILD_IDLE_SECONDS` | `vector_rebuild.idle_seconds` | `30` | アイドル秒数 |
| `NOUS_VECTOR_REBUILD_MIN_INTERVAL` | `vector_rebuild.min_interval` | `120` | 最小実行間隔（秒） |
| `NOUS_AUTO_CLEANUP_ENABLED` | `auto_cleanup.enabled` | `True` | 自動クリーンアップ |
| `NOUS_AUTO_CLEANUP_IDLE_MINUTES` | `auto_cleanup.idle_minutes` | `30` | アイドル分数 |
| `NOUS_AUTO_CLEANUP_CHECK_INTERVAL_SECONDS` | `auto_cleanup.check_interval_seconds` | `300` | チェック間隔（秒） |
| `NOUS_AUTO_CLEANUP_DUPLICATE_THRESHOLD` | `auto_cleanup.duplicate_threshold` | `0.90` | 重複検知閾値 |
| `NOUS_AUTO_CLEANUP_MIN_SIMILARITY_TO_REPORT` | `auto_cleanup.min_similarity_to_report` | `0.85` | レポート最小類似度 |
| `NOUS_AUTO_CLEANUP_AUTO_MERGE_ENABLED` | `auto_cleanup.auto_merge_enabled` | `False` | 自動マージ有効化 |
| `NOUS_AUTO_CLEANUP_AUTO_MERGE_THRESHOLD` | `auto_cleanup.auto_merge_threshold` | `0.95` | 自動マージ閾値 |
| `NOUS_AUTO_SUMMARIZATION_ENABLED` | `auto_summarization.enabled` | `True` | 自動要約スケジューラー |
| `NOUS_AUTO_SUMMARIZATION_SCHEDULE_DAILY` | `auto_summarization.schedule_daily` | `True` | 日次要約有効化 |
| `NOUS_AUTO_SUMMARIZATION_SCHEDULE_WEEKLY` | `auto_summarization.schedule_weekly` | `True` | 週次要約有効化 |
| `NOUS_AUTO_SUMMARIZATION_DAILY_HOUR` | `auto_summarization.daily_hour` | `3` | 日次要約実行時刻（時） |
| `NOUS_AUTO_SUMMARIZATION_WEEKLY_DAY` | `auto_summarization.weekly_day` | `0` | 週次要約実行曜日（0=月曜） |
| `NOUS_AUTO_SUMMARIZATION_CHECK_INTERVAL_SECONDS` | `auto_summarization.check_interval_seconds` | `3600` | チェック間隔（秒） |
| `NOUS_AUTO_SUMMARIZATION_MIN_IMPORTANCE` | `auto_summarization.min_importance` | `0.3` | 要約対象最小重要度 |
| `NOUS_AUTO_CLEANUP_CHECK_INTERVAL_SECONDS` | `auto_cleanup.check_interval_seconds` | `300` | チェック間隔（秒） |
| `NOUS_AUTO_CLEANUP_DUPLICATE_THRESHOLD` | `auto_cleanup.duplicate_threshold` | `0.90` | 重複判定閾値 |
| `NOUS_AUTO_CLEANUP_MIN_SIMILARITY_TO_REPORT` | `auto_cleanup.min_similarity_to_report` | `0.85` | 報告最小類似度 |
| `NOUS_AUTO_CLEANUP_MAX_SUGGESTIONS_PER_RUN` | `auto_cleanup.max_suggestions_per_run` | `20` | 実行あたり最大提案数 |
| `NOUS_AUTO_CLEANUP_AUTO_MERGE_ENABLED` | `auto_cleanup.auto_merge_enabled` | `False` | 自動マージ有効化 |
| `NOUS_AUTO_CLEANUP_AUTO_MERGE_THRESHOLD` | `auto_cleanup.auto_merge_threshold` | `0.95` | 自動マージ閾値 |
...
This document describes how to test Nous locally before deploying to production.
