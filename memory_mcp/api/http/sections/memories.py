    ...
    return """        <!-- ========== MEMORIES TAB ========== -->
        <section id="tab-memories" class="tab-panel" role="tabpanel">
            <div style="margin-bottom:16px; padding-bottom:12px; border-bottom:1px solid var(--glass-border);">
                <h2 style="font-size:1.25rem; font-weight:700; color:var(--text-primary); display:flex; align-items:center; gap:10px;"><span style="font-size:1.4rem;">🧠</span> Memories</h2>
            </div>
            <style>
...
def render_memories_js() -> str:
    ...
    return (
        ...
        "                body_state: m.body_state || null, emotions: m.emotions || null,\n"
        "                state_snapped_at: m.state_snapped_at || '',\n"
        "                created_at: m.created_at || '', updated_at: m.updated_at || '',\n"
        "                _score: m._score != null ? m._score : null,\n"
        "                _source: m._source || ''\n"
        "            }));\n"
        "            /* Compact body state indicator (fatigue/warmth only) */\n"
        "            var bodyCompactHtml = '';\n"
        "            if (m.body_state && (m.body_state.fatigue != null || m.body_state.warmth != null)) {\n"
        "                var bf = m.body_state.fatigue != null ? Math.round(m.body_state.fatigue * 100) : 0;\n"
        "                var bw = m.body_state.warmth != null ? Math.round(m.body_state.warmth * 100) : 0;\n"
        "                bodyCompactHtml = '<span style=\\\"font-size:0.65rem;color:var(--text-muted)\\\">';\n"
        "                if (bf > 30) bodyCompactHtml += '🔥' + bf + '% ';\n"
        "                if (bw > 50) bodyCompactHtml += '🌸' + bw + '%';\n"
        "                bodyCompactHtml += '</span>';\n"
        "            }\n"
        "\n"
        ...
        "            html += '<span class=\\\"mem-compact-meta\\\" style=\\\"font-size:0.72rem;color:var(--text-muted);min-width:50px\\\">' + bodyCompactHtml + ' ' + timeStr + '</span>';\n"
        "            html += '</div>';\n"
        "        });\n"
        "    } else {\n"
        "        /* ── Card view ── */\n"
        ...
        "            /* Compact body state indicator for card view */\n"
        "            var bodyCardHtml = '';\n"
        "            if (m.body_state && (m.body_state.fatigue != null || m.body_state.warmth != null)) {\n"
        "                var bf = m.body_state.fatigue != null ? Math.round(m.body_state.fatigue * 100) : 0;\n"
        "                var bw = m.body_state.warmth != null ? Math.round(m.body_state.warmth * 100) : 0;\n"
        "                bodyCardHtml = '<span style=\\\"font-size:0.7rem;color:var(--text-muted)\\\">';\n"
        "                if (bf > 30) bodyCardHtml += '🔥' + bf + '% ';\n"
        "                if (bw > 50) bodyCardHtml += '🌸' + bw + '%';\n"
        "                bodyCardHtml += '</span>';\n"
        "            }\n"
        "            var memJson = encodeURIComponent(JSON.stringify({\n"
        "                memory_key: key, content: m.content || '',\n"
        ...
        "            html += '<div class=\\\"memory-meta\\\">' + tagsHtml + ' ' + emoHtml + ' ' + strHtml + ' ' + scoreHtml + ' ' + bodyCardHtml + ' ' + timeHtml + '</div>';\n"
        "            html += '</div>';\n"
        "        });\n"
        "    }\n"
        ...
        "    }\n"
        "\n"
        "    /* Multi-dimensional Emotions bars */\n"
        ...
        '            h += \'<div class=\\"mem-modal-row\\"><span class=\\"mem-modal-key\\">Emotions</span><span style=\\"display:flex;flex-direction:column;gap:6px;flex:1\\">\';\n'
        "            emoEntries.forEach(function(e) {\n"
        "                var name = e[0], val = e[1];\n"
        "                var color = emoBarColors[name] || emoBarColors.neutral;\n"
        "                var pct = Math.round(val * 100);\n"
        "                h += '<div style=\\\"display:flex;align-items:center;gap:8px\\\">';\n"
        "                h += '<span style=\\\"font-size:0.75rem;color:var(--text-muted);min-width:70px;text-transform:capitalize\\\">' + esc(name) + '</span>';\n"
        "                h += '<div style=\\\"flex:1;height:5px;background:rgba(255,255,255,0.1);border-radius:3px;overflow:hidden\\\">';\n"
        "                h += '<div style=\\\"height:100%;width:' + pct + '%;background:' + color + ';border-radius:3px\\\"></div>';\n"
        "                h += '</div>';\n"
        "                h += '<span style=\\\"font-size:0.75rem;color:var(--text-muted);min-width:32px;text-align:right\\\">' + pct + '%</span>';\n"
        "                h += '</div>';\n"
        "            });\n"
        ...
        '        h += \'<div class="mem-modal-row"><span class="mem-modal-key">State</span><span>📸 \' + relativeTime(mem.state_snapped_at)'
