...
def render_persona_js() -> str:
    ...
    return (
        ...
        "            var emotions = ctx.emotions || {};\n"
        "            var emoji = emotionEmoji[emotionType] || '😐';\n"
        ...
        "                ? '<button onclick=\"switchPersonaTo(\\'' + esc(name) + '\\')\" class=\"glass-btn\" style=\"padding:6px 14px;font-size:0.85rem\">🔄 Switch</button>'\n"
        "                : '';\n"
        ...
        "                + '<div>📝 <strong>' + memCount + '</strong> memories</div>'\n"
        "                + '<div>💭 Last: ' + (lastConv ? relativeTime(lastConv) : 'Never') + '</div>'\n"
        "                + (function() { var topEmotions = Object.entries(emotions).filter(function(e) { return e[1] > 0.05; }).sort(function(a,b) { return b[1]-a[1]; }).slice(0, 3); return topEmotions.length > 0 ? '<div>' + topEmotions.map(function(e) { return e[0] + ':' + e[1].toFixed(1); }).join(' ') + '</div>' : '<div>' + emoji + ' ' + esc(emotionType) + ' (' + emotionIntensity.toFixed(1) + ')</div>'; })()\n"
        "                + (function() { \n"
        "                    var parts = []; \n"
        ...
        "                + switchBtn\n"
        "                + '<button onclick=\"deletePersona(\\'' + esc(name) + '\\')\" class=\"glass-btn\" style=\"' + deleteStyle + '\"' + deleteDisabled + '>🗑️ Delete</button>'\n"
        "                + '</div></div>';\n"
        "        });\n"
        "        grid.innerHTML = html;\n"
        "    } catch (e) {\n"
