...
def _auto_detect_host_data_root(data_root: str) -> str:
    ...
    try:
        ...
        logger.warning(
            ...
            "Set NOUS_SANDBOX__HOST_DATA_ROOT to the host-side path of '%s' "
...
def _build_container_configs(persona: str) -> tuple[dict, Path | None]:
    ...
    The volume mount key is resolved in this order:
      1. Explicit NOUS_SANDBOX__HOST_DATA_ROOT env var
    ...
    if not host_root:
        logger.warning(
            "Sandbox files will NOT persist on host. Set NOUS_SANDBOX__HOST_DATA_ROOT "
            "to the host-side path (e.g. /volume1/docker/Nous/data). "
