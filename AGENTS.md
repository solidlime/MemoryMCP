...
## ドキュメント更新ルール（必須）
- `nous/` 配下のコード（テスト・マイグレーション・`__init__.py` を除く）を変更した場合、以下のいずれかを必ず行うこと：
  1. 関連する `docs/*.md` を更新
  2. `README.md` を更新
  3. `CLAUDE.md` を更新（AIコーディングアシスタント向け情報の変更時）
  4. 上記いずれも不要な場合、コミットメッセージに `[skip-docs]` を含めて意図を明示する
- 判断基準：「この変更はユーザーに見えるか？」→ Yesなら `docs/` or `README`、Noなら `CLAUDE.md` or `[skip-docs]`
