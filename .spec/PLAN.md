...
## 2026-05-17: 実運用フィードバックによる修正（本セッション）

### 🔴 最重要: 感情モデルのロールバック
- 多次元感情（emotions: dict[str, float]）→ 単一感情タグ（emotion: str）+ intensity: float に戻す
- commit 57f3734 以前の仕様に戻す
- 時間経過でニュートラルに戻る減衰仕様は維持

### 🟡 Goal/Promise 無限増殖対策
- 完了・キャンセルしたgoal/promiseはmemoryに押し込む（アーカイブ）
- アクティブなgoal/promiseは定期的にLLMに注入（context injection）
- 「これからやること」等の安易なpromise追加を抑制する仕組みが必要
- 追加後に放置（完了にしない）問題への対策

### 🟡 WebUI Action 廃止
- Action（行動量）はPhysical+Mentalと同義。更新頻度低い
- バックエンド・フロントエンド両方から削除

### 🟡 Speech 更新促進
- 有用なコンテキストだが更新が疎か
- 定期的なリマインド or 自動的な更新促進が必要

### 🟠 Itemツール（built-in版）動作確認
- built-in版のitemツールが動作していない可能性
- MCPツール版はテスト済みで正常
- 🎒装備欄も動作していない可能性

### 🟠 Skills DB 定期更新
- スキル追加/削除しても skillsのDBが更新されない
- サーバー再起動なしで定期監視・自動更新が必要

### 🔴 身体状態の減衰処理不具合
- 時間経過しても身体状態がニュートラルに戻っていない
- 反映されていないのか、減衰処理そのものが漏れているのか調査が必要

### 🟡 WebUI メモリー詳細カードに身体状態未表示
- memories, timeline, graph 各セクションで body_state がカードに表示されていない

### 🟡 同一身体状態での記憶リンク確認
- emotion と同じように同一身体状態での記憶同士がリンクされる仕様のはず
- 実際に有効か検証

### 🟢 チャットUI改善
- 🧠記憶活動 / 🔧操作ログ：個別ログにマウスオーバーで詳細表示
- 記憶活動にメンタルモデルも表示
- 🎯アクティブな目標 / 🤝アクティブな約束 / メンタルモデル追加時も 💾保存された記憶 に表示
- ✨リフレクションは記憶ではない → 表示カテゴリ見直し
