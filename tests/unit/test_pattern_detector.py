...
def _make_memory(key: str, content: str, tags: list[str] | None = None, created_at: datetime | None = None, importance: float = 0.5) -> Memory:
...
class TestGetLastAbstractionAt:
    ...
    def test_returns_datetime_when_found(self):
        ...
        meta_mem = _make_memory(key="meta_1", content=f"last_decision_abstraction: {ts.isoformat()}", tags=[_MENTAL_MODEL_META_TAG])
        ctx = _make_mock_ctx()
    ...
    def test_returns_none_for_unrelated_type(self):
        meta_mem = _make_memory(key="meta_1", content="last_decision_abstraction: 2025-06-01T12:00:00", tags=[_MENTAL_MODEL_META_TAG])
        ctx = _make_mock_ctx()
        _set_get_by_tags(ctx, {"_meta": [meta_mem]})
    ...
    def test_invalid_isoformat_returns_none(self):
        meta_mem = _make_memory(key="meta_1", content="last_decision_abstraction: not-a-date", tags=[_MENTAL_MODEL_META_TAG])
        ctx = _make_mock_ctx()
        _set_get_by_tags(ctx, {"_meta": [meta_mem]})
        result = _get_last_abstraction_at(ctx, "decision")
        assert result is None
...
class TestStoreLastAbstractionAt:
    ...
    def test_replaces_existing_meta(self):
        old_meta = _make_memory(key="old_meta", content="last_decision_abstraction: 2024-01-01T00:00:00", tags=[_MENTAL_MODEL_META_TAG])
        ctx = _make_mock_ctx()
    ...
    def test_replaces_only_matching_type(self):
        old_meta_1 = _make_memory(key="meta_1", content="last_decision_abstraction: 2024-01-01T00:00:00", tags=[_MENTAL_MODEL_META_TAG])
        old_meta_2 = _make_memory(key="meta_2", content="last_preference_abstraction: 2024-01-01T00:00:00", tags=[_MENTAL_MODEL_META_TAG])
        ctx = _make_mock_ctx()
...
class TestHasNewMemoriesSince:
    ...
    def test_new_memory_returns_true(self):
        ...
        meta_mem = _make_memory(key="meta", content=f"last_decision_abstraction: {(now - timedelta(hours=1)).isoformat()}", tags=[_MENTAL_MODEL_META_TAG])
    ...
    def test_only_old_memories_returns_false(self):
        now = datetime.now().astimezone()
        meta_mem = _make_memory(key="meta", content=f"last_decision_abstraction: {now.isoformat()}", tags=[_MENTAL_MODEL_META_TAG])
...
class TestMaybeRunMentalModel:
    ...
    async def test_calls_llm_and_stores_results(self):
        ...
        now = datetime.now()
        decision_mems = [
            _make_memory(f"dec_{i}", f"decision content {i}", tags=["decision"], created_at=now - timedelta(hours=i), importance=0.7)
            for i in range(3)
        ]
        ...
        meta_calls = [call for call in ctx.memory_service.create_memory.call_args_list if call[1].get("tags") == [_MENTAL_MODEL_META_TAG]]
    ...
    async def test_does_not_reprocess_already_abstracted_groups(self):
        ...
        decision_mems = [
            _make_memory(f"dec_{i}", f"decision content {i}", tags=["decision"], created_at=now - timedelta(hours=2), importance=0.7)
            for i in range(3)
        ]
    ...
    async def test_empty_result_on_llm_failure(self):
        ...
        with patch("memory_mcp.application.chat.pattern_detector.get_provider", side_effect=Exception("LLM unavailable")):
            result = await maybe_run_mental_model(ctx, config)
        assert result == []
    ...
    async def test_multiple_type_groups_processed(self):
        ...
        for tag in _TYPE_TAGS:
            mapping[tag] = [
                _make_memory(f"{tag}_{i}", f"{tag} content {i}", tags=[tag], created_at=now - timedelta(hours=i), importance=0.6)
                for i in range(3)
            ]
        _set_get_by_tags(ctx, mapping)
